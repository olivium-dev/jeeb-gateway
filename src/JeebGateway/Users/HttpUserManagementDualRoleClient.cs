...
public sealed class HttpUserManagementDualRoleClient : IUserManagementDualRoleClient
{
    ...
    public async Task<PhoneFindOrCreateResult> PhoneFindOrCreateAsync(string phone, CancellationToken ct)
    {
        ...
        // The UM contract (FindOrCreateResponse) includes available_roles + active_role.
        // Use the real roles when UM returns them — this is the KYC real-status gate:
        // a user whose KYC was approved has "driver" in their available_roles and must
        // get that role in their JWT on every subsequent OTP login, NOT just "customer".
        // New users (isNew=true, or null/empty roles from UM) default to [customer]/customer.
        var opaqueRoles = dto.AvailableRoles is { Length: > 0 }
            ? dto.AvailableRoles
            : new[] { Roles.Client };
        var opaqueActive = !string.IsNullOrWhiteSpace(dto.ActiveRole)
            ? dto.ActiveRole!
            : Roles.Client;
    ...
    private sealed class PhoneFindOrCreateBodyResponse
    {
        ...
        // UM returns available_roles + active_role on find-or-create (per OpenAPI contract).
        // These carry the user's real KYC-granted roles (e.g. "driver") so a jeeber who
        // logs in via OTP receives their full role set, not just "customer".
        [JsonPropertyName("available_roles")] public string[]? AvailableRoles { get; set; }
