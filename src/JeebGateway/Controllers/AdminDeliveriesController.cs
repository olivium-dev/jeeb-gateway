...
public sealed class AdminDeliveriesController : ControllerBase
{
    ...
    private async Task<SourceResult> FetchSettlementsAsync(string deliveryId, CancellationToken ct)
    {
        ...
        if (client.BaseAddress is null)
            return new SourceResult(null, StatusCodes.Status503ServiceUnavailable, "not_configured");

        using var message = new HttpRequestMessage(HttpMethod.Get,
            $"admin/v1/settlements?deliveryId={Uri.EscapeDataString(deliveryId)}&limit=20");
        if (!UserIdentity.TryGetUserId(HttpContext, out var adminId, out _))
            return new SourceResult(null, StatusCodes.Status401Unauthorized, "admin_identity_missing");
        message.Headers.TryAddWithoutValidation("X-Admin-Id", adminId);
