...
public static class ServiceClientExtensions
{
        ...
        IHostEnvironment? environment = null)
    {
        ...
        // thin-BFF fan-out (3 of 4): the sole transcription runtime boundary is the
        // voice-transcription-service (FastAPI, host port 10062, health /healthz,
        // ready /readyz). The gateway owns no provider client, API key, audio store,
        // circuit breaker, or retry queue. BindBaseAddress resolves
        // Services:VoiceTranscription[:BaseUrl] with a trailing slash so the
        ...
        // until its DB became unavailable when 192.168.2.20 was decommissioned and 500'd /v1/users/me (D1).
        // The host was reactivated for staging on 2026-08-18; role-service remains retired. Roles are UM's. Do NOT re-add.
