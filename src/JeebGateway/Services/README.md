...
|---|---|---|---|
| **Generated** | `Services/Generated/` | NSwag (`nswag run <config>`) | NEVER |
| **Wrapper / hand-written** | `Services/` (this folder) | Hand-written adapters | Yes |

> `scripts/regenerate-clients.sh` is referenced here and in `contracts/README.md`
> but does **not** exist. The repo root holds `nswag-otp.json` and
> `nswag-state.json`; other clients are regenerated by hand.
...
will be lost the next time the client is regenerated.
...
Prefer `AddDownstreamClients`. `Program.cs` does hold about two dozen direct
`AddHttpClient` calls — named clients for state-service, wallet-ledger, tiers and
similar seams, several attaching resilience explicitly via
`ServiceClientExtensions.AttachResilienceOnly`. That set should not grow: a new
downstream client belongs in the extension.

## Why generated clients are committed

See [`contracts/README.md`](../contracts/README.md). Short version:
contract drift must be reviewable; builds must be hermetic; offline builds must
work.
