...
public sealed class AdminOidcFlowTests
{
    ...
    [Fact]
    public async Task ProductionStartupAndReadinessFailClosedWithoutExternalIdentity()
    {
        var production = new TestingEnvironment { EnvironmentName = "Production" };
        var missing = Configuration(new Dictionary<string, string?>());

        Action start = () => AdminOidcStartupGuard.EnsureConfigured(missing, production);
        start.Should().Throw<InvalidOperationException>().WithMessage("*not ready*");

        var unhealthy = await new AdminOidcConfigurationHealthCheck(missing, production)
    ...
    [Fact]
    public void ProductionControllerAllowListKeepsOidcRoutesReachable()
    {
        var feature = new ControllerFeature();
        feature.Controllers.Add(typeof(AdminOidcAuthController).GetTypeInfo());

        new EssentialGatewayControllerFeatureProvider().PopulateFeature([], feature);

        feature.Controllers.Should().Contain(typeof(AdminOidcAuthController).GetTypeInfo());
    }
