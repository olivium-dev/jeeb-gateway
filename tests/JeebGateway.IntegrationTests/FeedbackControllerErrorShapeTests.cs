...
/// <summary>
/// JEBV4-252 (info-leak): FeedbackController carried 6 bare
...
/// feedback-service response body, so every feedback endpoint leaked upstream
/// exception detail to the caller. The fix routes every catch through
/// <c>FeedbackController.UpstreamProblem</c>, a sanitized RFC 7807 ProblemDetails:
/// the upstream status is preserved, the upstream message/body is logged
/// server-side only and never put on the wire.
///
/// <para>Mirrors <see cref="ChatControllerErrorShapeTests"/> (the JEBV4-242
/// precedent). The public <c>/api/Feedback/comment</c> read passthrough exercises
/// the exact <c>catch (ApiException) → UpstreamProblem</c> path shared by all 6
/// feedback actions, driven by a raw HttpClient stub bound directly to the scoped
/// <see cref="ServiceFeedbackClient"/>.</para>
/// </summary>
public class FeedbackControllerErrorShapeTests
{
    private const string Canary =
        "System.NullReferenceException: SECRET_CANARY_f252 at FeedbackService.Internal.SecretRepo.Load() line 42";

    // A non-empty tag is required to pass the in-action guard and reach the upstream call.
    private const string Probe = "/api/Feedback/comment?tag=canary-probe";

    [Fact]
    public async Task GetComments_UpstreamServerError_Is_Sanitized_ProblemDetails_Not_Leaked_Message()
    {
        ...
        problem.Title.Should().Be("The feedback request could not be completed.");

        var raw = await resp.Content.ReadAsStringAsync();
        raw.Should().NotContain("SECRET_CANARY_f252",
            "the upstream response body must never reach the client (JEBV4-252)");
    ...
    [Fact]
    public async Task GetComments_UpstreamNotFound_Preserves_Status_And_Does_Not_Leak_Body()
    {
        var stub = new StubHttpMessageHandler(_ =>
    ...
    [Fact]
    public async Task GetComments_Upstream_Status_Outside_Error_Range_Is_Clamped_To_502()
    {
        var stub = new StubHttpMessageHandler(_ =>
    ...
    /// <c>catch (FeedbackApiException</c> paired 1:1 with <c>UpstreamProblem(ex)</c>.
    /// </summary>
    [Fact]
    public void FeedbackController_Source_Has_No_Live_Upstream_Passthrough_And_All_Catches_Are_Sanitized()
    {
        var path = LocateControllerSource("FeedbackController.cs");
        path.Should().NotBeNull(
            "src/JeebGateway/Controllers/FeedbackController.cs must be locatable from the test bin dir");
        ...
        CountOccurrences(liveCode, "StatusCode(ex.StatusCode").Should().Be(0,
            "JEBV4-252: no catch may return the upstream status with a raw ex.Message/body payload — "
            + "every upstream failure must route through UpstreamProblem");

        var catches = CountOccurrences(liveCode, "catch (FeedbackApiException");
    ...
    private static HttpClient MintBearerClient(WebApplicationFactory<Program> factory)
    {
        ...
        var token = new JwtSecurityToken(
            issuer: issuer,
            audience: audience,
            claims: new[] { new Claim("sub", "feedback-jebv4-252"), new Claim("roles", "client") },
