...
public sealed class AdminOpenApiContractTests
{
    ...
    public async Task RuntimeDocumentIsAuthoritativeForCookieAuthAndEssentialAdminModels()
    {
        ...
        paths.EnumerateObject().Select(property => property.Name).Should().NotContain(path =>
            path.StartsWith("/v1/admin/settlements/batches", StringComparison.Ordinal),
            "retired settlement aliases are not part of the lean CMS contract");
