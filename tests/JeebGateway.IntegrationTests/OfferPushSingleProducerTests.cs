...
/// synthesizing a 503, and <c>FeatureFlags:PushDispatchMode=upstream-authority</c> forces that
/// guard OFF (Program.cs refuses to boot otherwise), so the destination rung re-armed the
...
public class OfferPushSingleProducerTests
{
    ...
    /// <summary>
    /// The two rungs of FeatureFlags:PushDispatchMode with the direct-dispatch state each one
    /// implies — Program.cs Validate forces GatewayDirectDispatch:Enabled true at upstream-authority.
    /// </summary>
    ...
    [Fact]
    public async Task With_the_centre_switched_off_the_direct_client_is_still_the_fallback()
    {
        // The no-producer hole: durable write off leaves notification-service out of the path,
        // so the gateway must still send — one producer, never zero.
        var wire = new CountingHandler();
        var notifier = Notifier(new CountingHandler(), wire, directDispatchArmed: true, durableWrite: false);

        await notifier.NotifyNewOfferAsync(Context, Client, RequestId, OfferId, 1m, CancellationToken.None);

        wire.Posts.Should().Be(1, "with no upstream producer the gateway is the only one left");
    }

    [Fact]
    public async Task An_offer_with_no_durable_context_still_pushes_directly()
    {
        // No context => no record is built and nothing is handed over, so the fallback owns it.
        var centre = new CountingHandler();
        var wire = new CountingHandler();
        ...
        AttemptCountingHandler? outer = null)
    {
        var guard = new GatewayDirectPushDispatchGuardHandler(
            Options.Create(new GatewayDirectPushDispatchOptions { Enabled = directDispatchArmed }))
        {
            InnerHandler = wire,
        };
