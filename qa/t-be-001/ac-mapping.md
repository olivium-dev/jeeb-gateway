...
| **AC5** | "JWT is HS512; signing key 32+ bytes loaded from env; NEVER from `appsettings.json`." | `OtpVerifyTests.CorrectOtp_ReturnsJwtPair_AndFindOrCreatesUser` | `Otp/OtpVerifyTests.cs` | Asserts `SignatureAlgorithm == HS512`. Signing-key-source assertion is structural and verified by reading `IConfiguration` source (covered by Tech-Lead subtask JEB-465). |
...
| **AC-PhonePIIHash** | (same as above) | `PhonePiiScrubberTests.BcryptHash_IsPresent_InAtLeastOneSinkRecord` | `Otp/PhonePiiScrubberTests.cs` | Positive: at least one log record OR span tag matches a bcrypt prefix `$2[aby]$` — proves the hash IS recorded, not just "log nothing". |
...
| **AC-PhonePIIHash** | (same as above) | `PhoneNormalizationTests.EquivalentFormats_AllProduceSameBcryptHash` | `Otp/PhoneNormalizationTests.cs` | Theory: equivalent formats → identical bcrypt hash (proves normalisation precedes hashing). |
...
| **AC-ProblemTypeSet** *(new, from audit #14769)* | "The gateway only emits `type` strings from this frozen set on `/v1/auth/otp/{request,verify}`: `invalid_otp`, `too_many_attempts`, `invalid_country`, `rate_limited`, `invalid_phone`. Schemathesis enforces." | `CrossStoryMobileContractTests.EveryReturnedType_IsInTheFrozenSet` | `Otp/CrossStoryMobileContractTests.cs` | Drives all 5 error types in one test and verifies each observed `type` is in the frozen set. |
...
| **Total** | **31 test methods** (≥ 50 invocations when xUnit theories expand) |
