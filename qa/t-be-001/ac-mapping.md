...
**Source of truth precedence**: ADR-0012 for phone admission > audit comment #14769
(orchestrator reconciliation) > LEAD comment #14764 > UX #14761 > PO #14757 >
original JEB-37 description.
...
| **AC-PhoneNorm** *(superseded by ADR-0012)* | "Given an explicit international `+` phone, when the gateway processes it, then harmless presentation separators are canonicalized to the exact E.164 identity and valid countries are admitted by default." | `OtpPhonePolicyAndRateLimitTests.ValidNonLebanesePhone_IsAdmitted_Canonicalised_AndDialsUpstreamOnce` | `OtpPhonePolicyAndRateLimitTests.cs` | Positive all-country admission and canonical OTP boundary. |
| **AC-PhoneNorm** | (same as above) | `OtpPhonePolicyAndRateLimitTests.ValidLebanesePhone_IsAdmitted_AndDialsUpstreamOnce` | `OtpPhonePolicyAndRateLimitTests.cs` | Lebanon remains valid without being a server-side default. |
| **AC-PhoneNorm** | "National, `00`-prefixed, repaired, impossible, unknown-country, double-prefixed, or oversized input is rejected as `invalid_phone` before OTP cost." | `OtpPhonePolicyAndRateLimitTests.NonCanonicalOrImpossiblePhone_Returns400_AndNeverDialsUpstream` | `OtpPhonePolicyAndRateLimitTests.cs` | Table-driven fail-closed request boundary. |
| **AC-PhoneNorm** | (same as above) | `OtpPhonePolicyAndRateLimitTests.LibPhoneNumberRepair_IsRejectedByRequestAndVerifyWithoutUpstreamCalls` | `OtpPhonePolicyAndRateLimitTests.cs` | Proves libphonenumber trunk-prefix repair is rejected on request and verify. |
| **AC-PhoneNorm** | "When the emergency region restriction is enabled, a valid number outside `AllowedRegion` returns `invalid_country` without an OTP call." | `OtpPhonePolicyAndRateLimitTests.EmergencyRegionRestriction_WhenEnabled_RejectsNonLebanesePhone` | `OtpPhonePolicyAndRateLimitTests.cs` | Default deployment pins the restriction off. |
...
| **AC-ProblemTypeSet** | (same as above) | `OtpPhonePolicyAndRateLimitTests.EmergencyRegionRestriction_WhenEnabled_RejectsNonLebanesePhone` | `OtpPhonePolicyAndRateLimitTests.cs` | Per-type verification: `invalid_country` in emergency-restriction mode. |
