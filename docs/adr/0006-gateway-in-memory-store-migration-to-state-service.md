...
| `IPendingOffersStore` (~~in-memory impl~~ — **RETIRED, GW3 2026-08-01**) | ~~1490~~ | Row superseded, kept not deleted. The gateway's in-memory offer store was removed in GW3 / W3.5(c) and moved into the test project as a fixture double, so there is nothing here to migrate to the state-service: `IPendingOffersStore` now resolves only to the thin-BFF `UpstreamPendingOffersStore`, and the offer ledger of record is offer-service's own Postgres. The remaining gap is that the BFF is incomplete (get-by-id + bulk-withdraw routes missing upstream, JEBV4-148), which is an offer-service change, not a gateway KV migration. *Routing pair already durable via `StateServiceOfferRequestIndex`.* |
