...
class MintDevelopmentFirebaseDiagnosticProbeTests(unittest.TestCase):
    ...
    def test_script_never_persists_or_prints_request_material(self):
        ...
        self.assertIn('if hasattr(context, "keylog_filename"):\n        context.keylog_filename = None', text)
        self.assertIn("return run(sys.argv, sys.stdin, sys.stdout, sys.stderr)", text)
