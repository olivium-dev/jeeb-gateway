...
# Extraction adaptation (#364 carve-out): the full gateway keeps its in-memory
# fixture stores, so export under Testing instead of the essential-only
# Production profile; the emitted /admin/v1 subset is identical.
(cd "$scratch" && \
